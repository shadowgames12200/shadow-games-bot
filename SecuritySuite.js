...
async function applyQuarantine(member, cfg) {
  ...
  if (role && member.manageable) await member.roles.add(role, 'Aguardando verificação').catch(() => {});
}
...
async function handleMemberJoin(member, client) {
  ...
  if (cfg.blockBots && member.user.bot && !isStaff(member)) {
    const kicked = await safeKick(member, 'Bot bloqueado pela proteção do servidor').catch(() => false);
    await log(client, member.guild, `🤖 Bot **${member.user.tag}** ${kicked ? 'foi expulso' : 'não pôde ser expulso'} automaticamente.`, 0xe74c3c);
    return;
  }

  if (cfg.antiRaid && recent.length >= cfg.raidJoinLimit) {
    await log(client, member.guild, `🚨 Possível raid detectada: **${recent.length} entradas** em ${cfg.raidWindowMs / 1000}s.`, 0xe74c3c);
  }
...
async function handleMessage(message, client) {
  ...
  if (cfg.raidChannelId === message.channel.id && !isStaff(message.member)) {
    await message.delete().catch(() => {});
    const kicked = await safeKick(message.member, 'Mensagem enviada no canal reservado de proteção').catch(() => false);
    await log(client, message.guild, `🚨 ${message.author} enviou mensagem em <#${message.channel.id}> e ${kicked ? 'foi expulso' : 'não pôde ser expulso'}.`, 0xe74c3c);
    return;
  }
  ...
  if (recent.length >= cfg.spamLimit) {
    messages.delete(key);
    await message.member.timeout(60_000, 'Anti-spam automático').catch(() => {});
    await log(client, message.guild, `🛡️ ${message.author} recebeu timeout de 1 minuto por spam.`, 0xe67e22);
  }
...
async function handleInteraction(interaction, client) {
  ...
  if (!cfg.verificationEnabled || !cfg.memberRoleId) {
    return interaction.reply({ content: 'A verificação ainda não foi configurada.', ephemeral: true });
  }
  ...
  if (!role) return interaction.reply({ content: 'O cargo de membro configurado não existe mais.', ephemeral: true });
  const member = await interaction.guild.members.fetch(interaction.user.id);
  if (!member.manageable && role.position >= interaction.guild.members.me.roles.highest.position) {
    return interaction.reply({ content: 'O bot não consegue gerenciar esse cargo. Coloque o cargo do bot acima do cargo Membro.', ephemeral: true });
  }
...
async function install(client) {
  ...
  client.once('ready', () => console.log('[Security] suíte de segurança carregada.'));
}
...
async function postVerificationPanel(channel) {
  const embed = new EmbedBuilder()
    ...
    .setDescription('Clique no botão abaixo para confirmar que você é uma pessoa e liberar o acesso ao servidor.');
